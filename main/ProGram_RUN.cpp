...
void DistanceToGoal()
{
    ...
    DistanceNumberABS = sqrt((Target.x - Here.x*Target.x - Here.x)+
    (Target.y - Here.y*Target.y - Here.y));
    char Kyori[64];
    snprintf(Kyori, sizeof(Kyori), "ゴールとの絶対距離: %.2f.\n"
            , DistanceNumberABS);
...
void loop_main_RUN()
{
    ...
    if(ESP32S3 == false && DistanceABS >= 3.0)
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(1000));
         ...
         float DistanceABS = sqrt(distance_RUNNowANDthenLat*distance_RUNNowANDthenLat 
